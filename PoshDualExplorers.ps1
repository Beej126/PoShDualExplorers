...
function createButton {
    ...
    $faBtn.Button.Add_Click({ $action.Invoke($faBtn.Button) }.GetNewClosure() ) #pass pointer to wrappered button back into the action script to be able to change the icon upon state toggle
}
...
<#pointless w/o focus on the outer window
$frmMain.Add_KeyDown({
  $keyEventArgs = $_
